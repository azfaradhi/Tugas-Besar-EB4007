...
export default function LabRequestsPage() {
  ...
  const handleProcessTest = (test: LabTest) => {
    if (test.test_type === 'urin') {
      router.push(`/staff/lab/results?type=urin&id=${test.ID_hasil}`);
    } else if (test.test_type === 'ronsen') {
      router.push(`/staff/lab/results?type=ronsen&id=${test.ID_hasil}`);
    }
  };
  ...
  const filteredTests = labTests.filter((test) => {
    const matchesFilter =
      filter === 'all' || test.test_type === filter;
  ...
  const completedTests = filteredTests.filter((t) => hasResults(t));
